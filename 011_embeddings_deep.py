...
def basic_embeddings():
    ...
    single_embedding = embedder.embed_query(text)
...
def batch_embeddings():
    ...
    batch_embedding = embedder.embed_documents(text)
...
def similarity_search():
    ...
    doc_vector = embedder.embed_documents(docs)
...
def embedding_caching():
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        vectors1 = cached_embedder.embed_documents([text])
        print(f"  Embedded {len(vectors1)} documents")

        # Second call - from cache
        print("\nSecond call (Cache):")
        vectors2 = cached_embedder.embed_documents([text])
