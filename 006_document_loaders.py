...
# Silence the informational "langchain-community is being sunset" notice: these
# classic loaders still live there (langchain-classic only re-exports them with
# *more* deprecation warnings), so it stays their canonical home.
warnings.filterwarnings("ignore", message=".*langchain-community.*being sunset.*")
# WebBaseLoader reads USER_AGENT when it builds its HTTP session.
os.environ.setdefault("USER_AGENT", "agents-with-langchain-and-langgraph/0.1")
...
def load_text_file():
    with tempfile.NamedTemporaryFile(delete=False, suffix=".txt") as temp_file:
        temp_file.write(b"This is a sample file. This file is used to document loading...")
